...
namespace scottmasson\elephant\bytes\Text\Markdown;
class To
{
    const version = '1.7.4';
    ...
    protected function element(array $Element)
    {
        ...
        if ($Element['name'] === 'model') {
                ...
                */
                switch ($Element['type']) {
                    ...
                    case 'ARTICLE':
                         ...
                         *? @date 23/01/13 15:57 
                         */
                        $replace = $modelModule[$Element['type']]->markdown([
                            'sid'   =>  $Element['sid']
                        ]);
                        if ($replace !== false) {
                            $modelTemplate = file_get_contents($this->config->resource->path.$Element['type'].'.html');
            ...
            if ($this->config->safety->prevent->collection === false) {
                ...
                $markup = '<'.$Element['name'].' class="ui--'.md5((string)rand(1,2000)).'"';
            }
                ...
                if ($Element['name'] === 'p') {
                    ...
                    if ($this->config->ad !== false && !empty($this->config->ad['keys'][0])) {
                        if ($this->elementIncremental > $this->config->ad['keys'][0]) {
                            ...
                            if ($ad->display === true) {
                                $adModelTemplate = file_get_contents($this->config->resource->path.'ad-'.$ad->adsense.'-'.$ad->size.'.html');
